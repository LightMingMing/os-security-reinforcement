...
from system_config import Prop, show_colorful, padding
...
if __name__ == "__main__":
    ...
    # 连接数
    props = [Prop("打开文件数", "/etc/security/limits.conf", "*  -  nofile", "65536", " ", " +", 4)]
    files = ls('/etc/security/limits.d/*-nproc.conf')
    for filepath in files:
        filename = filepath[filepath.rfind('/') + 1:]
        props.append(Prop("普通用户连接数(" + filename + ")", filepath, "* soft nproc", "5120", " ", " +", 4))
        props.append(Prop("root用户连接数(" + filename + ")", filepath, "root soft nproc", "unlimited", " ", " +", 4))
    show_colorful(props)

    # 账户安全
    show_colorful([
        Prop("超时时间", "/etc/profile", "expoort TMOUT", "180", "=", "="),
    ...
    show_colorful([
        Prop("输出历史命令记录数", "/etc/ssh/sshd_config", "HISTSIZE", "2", "=", "="),
        Prop("保留的历史命令总数", "/etc/ssh/sshd_config", "HISTFILESIZE", "yes", "=", "="),
